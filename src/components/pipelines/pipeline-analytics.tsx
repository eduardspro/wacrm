    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          ...
          tooltip="Total de negocios en este pipeline que no están marcados como Perdido. Los ganados se incluyen."
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-primary" />}
          label="Valor del Pipeline"
          value={formatCurrency(stats.totalValue, defaultCurrency)}
          tooltip="Suma de los valores de todos los negocios en este pipeline, excluyendo los marcados como Perdido."
        />
        <Metric
          icon={<Target className="h-4 w-4 text-blue-400" />}
          label="Tamaño promedio"
          value={formatCurrency(stats.avgValue, defaultCurrency)}
          tooltip="Valor del Pipeline dividido por Total Negocios — el valor promedio de un negocio no perdido."
        />
        <Metric
          icon={<TrendingUp className="h-4 w-4 text-purple-400" />}
          label="Valor ponderado"
          value={formatCurrency(stats.weightedValue, defaultCurrency)}
          tooltip="Ingreso esperado: valor de cada negocio × probabilidad de su etapa. Primera etapa ≈ 10%, etapas progresan hasta 90%, Ganado = 100%. Los perdidos se excluyen."
        />
        <Metric
